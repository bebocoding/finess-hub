from app.user.user_profile import UserProfile
from app.models.rule_based_model.rules import FitnessRules


class Assessment:
    def __init__(self, user_profile):
        self.user_profile = user_profile
        self.fitness_level = user_profile.fitness_level
        self.preferences = user_profile.preferences
        self.goals = user_profile.fitness_goals
    ...
    def _goal_setting(self):
        ...
        selected_goals_indices = input(
            "Enter the numbers corresponding to your chosen goals (comma-separated): ").split(',')
        selected_goals_indices = [int(index)
    ...
    def get_recommended_assessment_phases(self):
        ...
        if self.fitness_level in ["Overweight", "Obese"]:
            recommended_phases.extend(
                ["Strength Assessment", "Endurance Assessment", "Power Assessment"])
        ...
        if "Sports Performance Enhancement" in self.goals:
            recommended_phases.extend(
                ["Power Assessment", "Endurance Assessment"])

        if "Injury Prevention" in self.goals:
            recommended_phases.extend(
                ["Flexibility Assessment", "Stabilization Assessment"])

        if "Stress Reduction" in self.goals:
            recommended_phases.extend(
                ["Endurance Assessment", "Flexibility Assessment"])
    ...
    def recommend_fitness_phase(self, assessment_manager):
        ...
        recommended_phases = [
            phase for phase, criteria_met in phases_criteria.items() if criteria_met]
