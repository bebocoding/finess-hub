...
class StabilizationAssessment(OPTPhaseAssessment):
    ...
    def calculate_score(self):
        ...
        single_leg_balance_time = self.assessment_data.get(
            'single_leg_balance_time', 0)
        rotational_score = self.assessment_data.get('rotational_score', 0)

        # Calculate stabilization score based on the recorded data
        stabilization_score = (
            plank_time * 0.4) + (single_leg_balance_time * 0.3) + (rotational_score * 0.3)
    ...
    def run_assessment(self):
        ...
        plank_time = float(
            input("Enter the time (in seconds) you held the plank position: "))
        self.record_user_data({'plank_time': plank_time})

        # Collect user data for single-leg balance exercise
        single_leg_balance_time = float(
            input("Enter the time (in seconds) you held the single-leg balance: "))
        self.record_user_data(
            {'single_leg_balance_time': single_leg_balance_time})

        # Collect user data for rotational exercise
        rotational_score = float(
            input("Enter your score (0-10) for the rotational exercise: "))
