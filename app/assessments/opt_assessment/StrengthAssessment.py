...
class StrengthAssessment(OPTPhaseAssessment):
    ...
    def calculate_score(self):
        ...
        strength_score = (pushups * 0.2) + (squats * 0.3) + \
            (bench_press * 0.3) + (deadlift * 0.2)
    ...
    def run_assessment(self):
        ...
        bench_press = float(
            input("Enter the maximum weight (in lbs) you benched: "))
        self.record_user_data({'bench_press': bench_press})

        # Collect user data for deadlift
        deadlift = float(
            input("Enter the maximum weight (in lbs) you deadlifted: "))
