from app.opt_model.fitness_phase import FitnessPhase
from app.opt_model.training_parameters import TrainingParameters
...
class StabilizationEndurance(FitnessPhase):
    ...
    def flexibility(self):
        ...
        self.flexibility_training_parameters = TrainingParameters(muscle_groups="Specific", reps=(1, 3), sets=(1, 1), tempo=30, intensity="Low", load="Light", rest=(
            30, 60), schedule="Fixed", frequency=(3, 7), volume="Low", split="Full Body", duration=(self.duration), exercise_selection="SMR and static")

    def core(self):
        # Define specific core training plan for Stabilization Endurance phase
        self.core_training_parameters = TrainingParameters(muscle_groups="General", reps=(12, 20), sets=(1, 4), tempo="Slow 4/2/1", intensity="Moderate", load="Moderate", rest=(
            0, 90), schedule="Flexible", frequency=(2, 4), volume=None, split="Full Body", duration=(self.duration), exercise_selection="1–4 core-stabilization")

    def balance(self):
        # Define specific balance training plan for Stabilization Endurance phase
        self.balance_training_parameters = TrainingParameters(muscle_groups="General", reps=(12, 20), sets=(6, 10), tempo="Slow 4/2/1", intensity="Moderate", load="Moderate", rest=(
            0, 90), schedule="Flexible", frequency=(2, 4), volume=None, split="Full Body", duration=(self.duration), exercise_selection="1–4 balance-stabilization")

    def plyometric(self):
        # Define specific plyometric training plan for Stabilization Endurance phase (likely minimal or absent)
        self.plyometric_training_parameters = TrainingParameters(muscle_groups="General", reps=(5, 8), sets=(1, 3), tempo="3–5 s hold on landing", intensity="Moderate", load="Moderate", rest=(
            0, 90), schedule="Flexible", frequency=(4), volume=None, split="Full Body", duration=(4, 6), exercise_selection="0–2 plyometric-stabilization")

    def saq(self):
        # Define specific SAQ training plan for Stabilization Endurance phase (likely minimal or absent)
        self.saq_training_parameters = TrainingParameters(muscle_groups="General", reps=(2, 3), sets=(1, 2), tempo="Moderate", intensity="Moderate", load="Moderate", rest=(
            0, 90), schedule="Flexible", frequency=(2, 4), volume=None, split="Full Body", duration=(self.duration), exercise_selection="4–6 drills with limited horizontal inertia and unpredictability")

    def resistance(self):
        # Define specific resistance training plan for Stabilization Endurance phase, focusing on low weight, high reps
        self.resistance_training_parameters = TrainingParameters(muscle_groups="General", reps=(2, 3), sets=(1, 2), tempo="Moderate", intensity="Moderate", load="Low", rest=(
            0, 90), schedule="Flexible", frequency=(2, 4), volume=None, split="Full Body", duration=(self.duration), exercise_selection="Varied")
